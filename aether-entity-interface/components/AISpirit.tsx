...
class SimplexNoise {
  ...
  noise3D(x: number, y: number, z: number) {
    ...
    if(t0<0) n0 = 0.0; else { t0 *= t0; n0 = t0 * t0 * (this.grad3[gi0][0]*x0 + this.grad3[gi0][1]*y0 + this.grad3[gi0][2]*z0); }
    let t1 = 0.6 - x1*x1 - y1*y1 - z1*z1;
    if(t1<0) n1 = 0.0; else { t1 *= t1; n1 = t1 * t1 * (this.grad3[gi1][0]*x1 + this.grad3[gi1][1]*y1 + this.grad3[gi1][2]*z1); }
    let t2 = 0.6 - x2*x2 - y2*y2 - z2*z2;
    if(t2<0) n2 = 0.0; else { t2 *= t2; n2 = t2 * t2 * (this.grad3[gi2][0]*x2 + this.grad3[gi2][1]*y2 + this.grad3[gi2][2]*z2); }
    let t3 = 0.6 - x3*x3 - y3*y3 - z3*z3;
    if(t3<0) n3 = 0.0; else { t3 *= t3; n3 = t3 * t3 * (this.grad3[gi3][0]*x3 + this.grad3[gi3][1]*y3 + this.grad3[gi3][2]*z3); }
...
class Particle {
  ...
  ox: number; oy: number; oz: number; // Orbit anchors
  size: number;
  color: string;
  ...
  constructor(radius: number) {
    // Distribute in a cloud around the center
    ...
    const colors = [
        ...
        'rgba(200, 100, 255, 0.5)'  // Purple hint
    ];
  ...
  update(agitation: number, rotY: number, rotX: number, time: number) {
    if (this.isShooting) {
        // Shooting Star Physics (No Orbit)
        this.x += this.vx;
        this.y += this.vy;
        this.z += this.vz;
        this.life -= 0.02;
    ...
    // Reduced frequency by SIGNIFICANT amount as requested (Seldom)
    ...
    // 2. Drift / Float Away Logic
    // Some particles are looser
...
const AISpirit: React.FC<AISpiritProps> = ({ state, onInteraction }) => {
  ...
  const dragPosRef = useRef({ x: 0, y: 0 }); // Where the mouse IS currently (if dragging)
  const hoverPosRef = useRef({ x: -1000, y: -1000 }); // Where the mouse IS currently (always)
  const spiritPosRef = useRef({ x: 0, y: 0 }); // Where the spirit visual center is (Elastic)
  const agitationRef = useRef(0); // 0 to 1, increases when holding
  
  // --- Initialize High-Res Mesh ---
  useEffect(() => {
    const initGeometry = () => {
      const radius = 110;
      // Increased segments significantly for dense "Made of Particles" look
      ...
      // Reduced cloud particles count slightly to focus on blob
      for(let i=0; i<120; i++) {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (isDraggingRef.current) {
          // Drag logic: Pull center slightly towards mouse, but also deform mesh (see below)
          ...
          // Ramp up agitation quickly when interacting
      ...
      // SLOWED DOWN: Kept slow as requested
      ...
      // --- 1. Update Mesh (Liquid Distortion + Drag Deform) ---
      const transformedVertices: Vector3[] = [];
      
      const noiseFreq = 0.008 + (agitation * 0.005); 
      const baseAmp = state.isSpeaking ? 35 : 20;
      // const noiseAmp = baseAmp + (agitation * 15);
      ...
      meshRef.current.vertices.forEach(v => {
        ...
        // --- Interaction Logic (Ripple & Pull) ---
        
        // Calculate approx screen position of this vertex to test proximity
        const pScale = fov / (fov + z);
        const pX = cx + x * pScale;
        const pY = cy + y * pScale;
        
        // Distance from current mouse position (hover or drag)
        const dx = hoverPosRef.current.x - pX;
        const dy = hoverPosRef.current.y - pY;
        const distToMouse = Math.sqrt(dx*dx + dy*dy);
        
        // A. Hover Ripple ("Finger in water")
        // Sensitive radius approx 150px
        if (distToMouse < 150) {
            const influence = (1 - distToMouse / 150);
            
            // Create a sine wave ripple based on distance and time
            // Moving mouse changes 'distToMouse', causing the wave to move
            const rippleAmp = 0.15; // strength
            const rippleFreq = 0.1; // tightness of waves
            const rippleSpeed = 15; // speed of wave animation
        ...
        // B. Drag Indentation (Localized Grab)
        // If clicking, the pull is stronger and more "magnetic" at the cursor
        if (isDraggingRef.current) {
             const grabRadius = 200;
             if (distToMouse < grabRadius) {
                 const grabInfluence = Math.pow((1 - distToMouse / grabRadius), 2); // Quadratic falloff for sharper grab
                 
                 // Pull vertex towards mouse
                 // We convert mouse diff back to 'world'ish scale approx
                 x += dx * grabInfluence * 0.3; // 0.3 factor to control stretchiness
      ...
      // --- 2. Render Blob as PARTICLE SURFACE ---
      const renderList: { type: 'blob_particle' | 'cloud_particle', z: number, draw: () => void }[] = [];
      ...
      meshRef.current.faces.forEach(face => {
        ...
        const dotView = nnz; 
        const fresnel = Math.pow(1 - Math.abs(dotView), 2); // Squared for sharper edge

        let r, g, b, a;
        
        if (agitation > 0.3) {
            r = 100 + agitation * 155; g = 150 + agitation * 50; b = 255;
            a = (0.2 + fresnel * 0.8);
            ...
            const nnx = nx/len;
            const nny = ny/len;
            r = 40 + Math.abs(nnx) * 40;
            g = 120 + Math.abs(nny) * 60;
            b = 255;
            // Center particles are faint (ghostly), Edge particles are bright
            a = 0.1 + fresnel * 0.9; 
        }

        const p = project({x: cx3, y: cy3, z: cz3}, fov, cx, cy);
        const scale = fov / (fov + cz3);
  ...
  const handleStart = (e: React.MouseEvent | React.TouchEvent) => {
      ...
      updateMousePos(e); // Ensure position is set immediately on click
  ...
  const handleEnd = () => {
      ...
      // We do NOT reset hoverPosRef so the ripple can trail off naturally
  };
  
  // Set initial spirit position on mount
  useEffect(() => {
